...
DayA = A(:, 2);
A = A(:,[1 2]);
clearvars -except A AORD DayA %Remove all variable except A
...
%         if ((i==1) && (j==1)) || (bic<c)
%             c=aic; % <- change this line for AIC
%         end           
%             p=i;
%             q=j;
%     crit(i,j)=aic; % <- change this line for AIC
%     end
% end
% %In variable crit (p is rows q is columns)
...
% Md1 = arima(p,d,q);
% EstMdl = estimate(Md1,TrainSet(:, 1));
% [yF,yMSE] = forecast(EstMdl,T2,'Y0',ylog);
